...
def limpa_terminal():
    """Limpa o terminal com o comando adequado ao sistema operacional."""
...
def monta_linhas_mapa(matriz, esconder_navios):
    """
    Monta as linhas de texto do tabuleiro com bordas e coordenadas.

    Com esconder_navios=True, oculta as partes intactas ('O') na exibição,
    sem alterar a matriz original.
    """
...
def cria_navios():
    """Retorna a frota inicial, com um 'O' para cada parte intacta dos navios."""
...
def verifica_navio_afundado(partes):
    """Retorna True quando não restam partes intactas ('O') no navio."""
...
def verifica_frota_destruida(navios):
    """Retorna True quando todos os navios da frota estão afundados."""
...
def encontra_navio(navios, navios_adicionados, linha, coluna):
    ...
    # Cada registro guarda [linha inicial, coluna inicial, direção].
...
def escolhe_navio():
    ...
    Retorna '0' para voltar ao menu ou None se a opção não existir.
    """
...
def valida_linha(indice):
    """Verifica se o índice da linha está entre 0 e 9."""
...
def valida_coluna_letra(j):
    """Verifica se a entrada é uma letra minúscula de a até j."""
...
def verifica_vizinhanca(matriz, linha, coluna):
    ...
    # O bloco 3x3 inclui o centro, que será ignorado para consultar só os vizinhos.
...
def apaga_navio(matriz, navios_adicionados, navio, navios):
    """Limpa as posições do navio na matriz durante o reposicionamento."""
...
def escolhe_posicao(matriz, navios, navio, navios_adicionados):
    """Solicita uma posição válida, insere o navio e registra linha, coluna e direção."""
...
def busca_navio(matriz, linha, coluna):
    """Retorna True se a posição consultada contiver uma parte intacta ('O')."""
...
def recebe_ataque(matriz, navios, navios_adicionados, linha, coluna):
    """
    Aplica um tiro em uma posição válida e ainda não atacada.

    Marca 'X' no mapa e na frota em caso de acerto, ou '~' em caso de erro.
    Retorna linha, coluna, acertou, navio e afundou em um dicionário.
    Quem chama a função deve verificar os limites e impedir tiros repetidos.
    """
    ...
    # Atualiza o mapa e a frota para manter os acertos sincronizados.
...
def sorteia(lista):
    """Retorna um elemento aleatório de uma lista não vazia."""
...
def escolhe_alvo_bot(matriz, estado):
    """
    Escolhe as coordenadas do próximo tiro usando o histórico de ataques.

    Após um acerto, procura nas casas vizinhas; com dois ou mais acertos,
    segue pelas pontas na direção do navio. Na busca por novos navios,
    prioriza casas alternadas, pois todo navio de tamanho 2 ou maior
    ocupa pelo menos uma delas. Pressupõe que ainda exista um alvo livre.
    """
        ...
        for c in range(len(matriz[l])):
            if posicao_livre(matriz, l, c):
                ...
                if estado["descartadas"][l][c] == "":
                    livres.append([l, c])
                    # A soma par seleciona casas alternadas, como em um xadrez.
                    if (l + c) % 2 == 0:
                        xadrez.append([l, c])
...
def atualiza_estado_bot(estado, resultado):
    ...
    if resultado["acertou"]:
        ...
        if resultado["afundou"]:
            # Navios não se encostam: os arredores do navio afundado são descartados.
...
def mostra_tabuleiros(matriz_jogador, navios_jogador, matriz_bot, navios_bot,
                      revela_bot):
    """
    Exibe os mapas e as frotas do jogador e do bot lado a lado.

    Com revela_bot=False, esconde os navios intactos e os danos parciais
    na lista da frota adversária. Os tiros já realizados continuam visíveis.
    """
